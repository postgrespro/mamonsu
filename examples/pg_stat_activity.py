...
class SparkmesActivity(Plugin):
    ...
    #   0      1         2         3      4          5             6
    Items = [
        ('calls_upd', 'pgsql.queries.sparkmes', 'число запросов (modify)', 'n/m',
         Plugin.DELTA.simple_change, 1, Plugin.VALUE_TYPE.numeric_unsigned),
        ('calls_sel', 'pgsql.queries.sparkmes', 'число запросов (select)', 'n/m',
         Plugin.DELTA.simple_change, 1, Plugin.VALUE_TYPE.numeric_unsigned),

        ('time_upd', 'pgsql.queries.sparkmes', 'время (modify)', Plugin.UNITS.ms,
         Plugin.DELTA.simple_change, 2, Plugin.VALUE_TYPE.numeric_float),
        ('time_sel', 'pgsql.queries.sparkmes', 'время (select)', Plugin.UNITS.ms,
         Plugin.DELTA.simple_change, 2, Plugin.VALUE_TYPE.numeric_float),

        ('callsfirm', 'pgsql.queries.sparkmes', 'число запросов (firm)', 'n/m',
         Plugin.DELTA.simple_change, 1, Plugin.VALUE_TYPE.numeric_unsigned),
        ('callsip_s', 'pgsql.queries.sparkmes', 'число запросов (ip)', 'n/m',
         Plugin.DELTA.simple_change, 1, Plugin.VALUE_TYPE.numeric_unsigned),

        ('timefirm', 'pgsql.queries.sparkmes', 'время (firm)', Plugin.UNITS.ms,
         Plugin.DELTA.simple_change, 2, Plugin.VALUE_TYPE.numeric_float),
        ('timeip_s', 'pgsql.queries.sparkmes', 'время (ip)', Plugin.UNITS.ms,
         Plugin.DELTA.simple_change, 2, Plugin.VALUE_TYPE.numeric_float),
    ]
